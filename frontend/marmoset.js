...
(function (marmoset) {
  ...
  function Archive(a) {
    ...
    for (a = new ByteStream(a); !a.empty();) {
      ...
      var c = a.readUint32(), d = a.readUint32(), e = a.readUint32();
  ...
  Archive.prototype.checkSignature = function (a) {
    ...
    a.setBytes([0, 233, 33, 170, 116, 86, 29, 195, 228, 46, 189, 3, 185, 31, 245, 19, 159, 105, 73, 190, 158, 80, 175, 38, 210, 116, 221, 229, 171, 134, 104, 144, 140, 5, 99, 255, 208, 78, 248, 215, 172, 44, 79, 83, 5, 244, 152, 19, 92, 137, 112, 10, 101, 142, 209, 100, 244, 92, 190, 125, 28, 0, 185, 54, 143, 247, 49,
      37, 15, 254, 142, 180, 185, 232, 50, 219, 11, 186, 106, 116, 78, 212, 10, 105, 53, 26, 14, 181, 80, 47, 87, 213, 182, 19, 126, 151, 86, 109, 182, 224, 37, 135, 80, 59, 22, 93, 125, 68, 214, 106, 209, 152, 235, 157, 249, 245, 48, 76, 203, 0, 0, 95, 200, 246, 243, 229, 85, 79, 169], !0);
    d = new BigInt;
    d.setBytes(b[0]);
    return d.powmod(65537, a).toInt32() != c ? !1 : !0
  };
  Archive.prototype.decompress = function (a, b) {
    var c = new Uint8Array(b), d = 0, e = new Uint32Array(4096), f = new Uint32Array(4096), g = 256, h = a.length,
      k = 0, l = 1, m = 0, n = 1;
    c[d++] = a[0];
    for (var r = 1; ; r++) {
      n = r + (r >> 1);
      if (n + 1 >= h) break;
      var m = a[n + 1], n = a[n], p = r & 1 ? m << 4 | n >> 4 : (m & 15) << 8 | n;
      if (p < g) if (256 > p) m = d, n = 1, c[d++] = p; else for (var m = d, n = f[p], p = e[p], q = p + n; p < q;) c[d++] = c[p++]; else if (p == g) {
        m = d;
        n = l + 1;
        p = k;
  ...
  BigInt.prototype.setBytes = function (a, b) {
    ...
    if (b) for (var d = 0, c = a.length - 1; 0 <= c; c -= 2) this.digits[d++] = a[c] + (0 < c ? 256 * a[c - 1] : 0); else for (d = 0; d < c; ++d) this.digits[d] = a[2 * d] + 256 * a[2 * d + 1];
    this.trim()
  };
  ...
  BigInt.prototype.lessThan = function (a) {
    if (this.digits.length == a.digits.length) for (var b = this.digits.length - 1; 0 <= b; --b) {
      var c = this.digits[b], d = a.digits[b];
      if (c != d) return c < d
    }
  ...
  BigInt.prototype.bitCount = function () {
    var a = 0;
    if (0 < this.digits.length) for (var a = 16 * (this.digits.length - 1), b = this.digits[this.digits.length - 1]; b;) b >>>= 1, ++a;
    return a
  };
  BigInt.prototype.sub = function (a) {
    var b = this.digits, c = a.digits, d = this.digits.length;
    a = a.digits.length;
    for (var e = 0, f = 0; f < d; ++f) {
      var g = b[f], h = f < a ? c[f] : 0, h = h + e, e = h > g ? 1 : 0, g = g + (e << 16);
      b[f] = g - h & 65535
    }
    this.trim()
  };
  ...
  function Bounds(a) {
    for (var b = 0; b < a.length; ++b) {
      var c = a[b].bounds;
      if (void 0 === this.min) this.min = [c.min[0], c.min[1], c.min[2]], this.max = [c.max[0], c.max[1], c.max[2]]; else for (var d = 0; 3 > d; ++d) this.min[d] = Math.min(c.min[d], this.min[d]), this.max[d] = Math.max(c.max[d], this.max[d])
    }
  ...
  ByteStream.prototype.readCString = function () {
    for (var a = this.bytes, b = a.length, c = 0; c < b; ++c) if (0 == a[c]) return a = String.fromCharCode.apply(null, this.bytes.subarray(0, c)), this.bytes = this.bytes.subarray(c + 1), a;
    return null
  };
  ...
  ByteStream.prototype.readUint32 = function () {
    var a = this.bytes, b = a[0] | a[1] << 8 | a[2] << 16 | a[3] << 24;
  ...
  var prepareEmbedParams = function (a) {
    ...
    embed = function (a, b) {
      ...
      if (b.pagePreset) {
        ...
        if (!c.mobile) {
          ...
          var g = [e.style.width, e.style.height], h = function () {
            if (FullScreen.active()) e.style.resize = "none"; else if (e.style.resize = "both", g[0] != e.style.width || g[1] != e.style.height) g[0] = e.style.width, g[1] = e.style.height, c.resize(e.clientWidth - 12, e.clientHeight - 6 - 16);
    ...
    console.log("fetchThumbnail");
      ...
      Network.fetchBinaryIncremental(a + e + "thumb=1", function (a) {
        console.log("function(a) work!");
        // (a = (new Archive(a)).extract("thumbnail.jpg")) ? TextureCache.parseFile(a,
        //   b, d) : c && c();
        (a = (new Archive(a)).extract("thumbnail.jpg")) ? TextureCache.parseFile1(a,
          b) : c && c();
        console.log(a);
        return 0
      }, c, 71680)
  ...
  Input.prototype.attach = function (a) {
    ...
    this.mouseStates = [{pressed: !1, position: [0, 0], downPosition: [0, 0]}, {
      pressed: !1,
      position: [0, 0],
      downPosition: [0, 0]
    }, {pressed: !1, position: [0, 0], downPosition: [0, 0]}];
    this.lastTapPos = [0, 0];
    a = function (a) {
    ...
    a = function (a) {
      var d = this.mouseStates[a.button];
      if (d) {
        var e = this.element.getBoundingClientRect(), f = a.clientX - e.left, e = a.clientY - e.top;
        d.pressed = !1;
        d.position[0] = f;
        d.position[1] = e;
    ...
    a = function (a) {
      for (var d = !1, e = 0; 3 > e; ++e) {
        ...
        if (f.pressed) {
          var g = this.element.getBoundingClientRect(), d = a.clientX - g.left, g = a.clientY - g.top,
            h = d - f.position[0], k = g - f.position[1];
          f.position[0] = d;
          f.position[1] = g;
          if (1 <= e || a.ctrlKey) for (f = 0; f < this.onPan.length; ++f) this.onPan[f](h, k); else if (0 == e) if (a.shiftKey) for (f = 0; f < this.onPan2.length; ++f) this.onPan2[f](h, k); else for (f = 0; f < this.onDrag.length; ++f) this.onDrag[f](d, g, h, k);
          d = !0
        }
      }
      d && b(a)
    ...
    a = function (a) {
      for (var d = this.element.getBoundingClientRect(), e = !1, f = 0; f < a.changedTouches.length; ++f) if (a.target === this.element) {
        var g = a.changedTouches[f], e = {x: g.clientX - d.left, y: g.clientY - d.top};
        e.startX =
          e.x;
        e.startY = e.y;
    ...
    a = function (a) {
      for (var d = !1, e = 0; e < a.changedTouches.length; ++e) {
        var f = a.changedTouches[e], g = this.touches[f.identifier];
        if (g) {
          if (this.tapPossible) {
            var h = this.element.getBoundingClientRect(), d = f.clientX - h.left, h = f.clientY - h.top;
    ...
    a = function (a) {
      ...
      if (1 == d.length && 1 >= this.touchCountFloor) {
        var g = d[0], h = this.touches[g.identifier];
        if (h) {
          var k = g.clientX - f.left, g = g.clientY - f.top, f = k - h.x, l = g - h.y;
          h.x = k;
          h.y = g;
        ...
        if (l = d[0], e = this.touches[l.identifier], g = d[1], h = this.touches[g.identifier], e && h) {
          var k = l.clientX - f.left, l = l.clientY - f.top, m = g.clientX - f.left, n = g.clientY - f.top,
            r = Math.sqrt((k - m) * (k - m) + (l - n) * (l - n)), p = Math.sqrt((e.x - h.x) * (e.x - h.x) + (e.y -
            h.y) * (e.y - h.y)), q = Math.abs(r - p), f = (k - e.x + m - h.x) / 2, g = (l - e.y + n - h.y) / 2,
            u = Math.sqrt(f * f + g * g);
          e.x = k;
          e.y = l;
          h.x = m;
  ...
  Lights.prototype.update = function (a, b) {
    ...
    for (var d = 0; d < this.count; ++d) {
      var e = this.positions.subarray(4 * d, 4 * d + 4), f = this.directions.subarray(3 * d, 3 * d + 3),
        g = this.getLightPos(d), h = this.getLightDir(d);
    ...
    for (var f = new Float32Array(this.finalTransformBuffer), g = Matrix.empty(), h = Matrix.empty(), k = Matrix.empty(), l = Vect.empty(), m = Vect.empty(), n = Vect.empty(), r = Vect.empty(), e = Vect.empty(), p = [], q = [], u = Matrix.create(0.5, 0, 0, 0.5, 0, 0.5, 0, 0.5, 0, 0, 0.5, 0.5, 0, 0, 0, 1), d = 0; d < this.count; ++d) {
      ...
      var l = -p[2], m = -q[2], s = this.spot[3 * d];
    ...
    for (d = 0; d < this.shadowCount; d++) if (e && 1 == this.matrixWeights[d]) this.shadowsNeedUpdate[d] = 1; else for (c = 16 * d; c < 16 * d + 16; ++c) if (f[c] != this.finalTransformBuffer[c]) {
  ...
  function Material(a, b, c) {
    this.gl = a;
    this.name = c.name;
    var d = {mipmap: !0, aniso: a.hints.mobile ? 0 : 4, clamp: !!c.textureWrapClamp, mirror: !!c.textureWrapMirror},
      e = {mipmap: d.mipmap, clamp: d.clamp, mirror: d.mirror, nofilter: c.textureFilterNearest || !1};
    ...
    c.microfiber && (e.push("#define MICROFIBER"), this.microfiberParams = c.microfiberParams || {
      fresnelColor: [0.2, 0.2, 0.2, 0.5], fresnelOcc: 1,
      fresnelGlossMask: 1
  ...
  Material.prototype.bind = function (a) {
    ...
    var b = a.view, c = a.lights, d = a.sky, e = a.shadow, f = a.stripData.active() ? this.stripShader : this.shader,
      g = this.skinParams, h = this.anisoParams, k = this.microfiberParams, l, m = this.gl, n = f.params,
      r = this.textures, p = f.samplers;
    f.bind();
    this.blend();
    ...
    if (0 < c.count && (m.uniform4fv(n.uLightPositions, c.positionBuffer), m.uniform3fv(n.uLightDirections, c.directionBuffer), m.uniform3fv(n.uLightColors, c.colors), m.uniform3fv(n.uLightParams,
      ...
      var q = e.depthTextures[0].desc.width, u = e.depthTextures[0].desc.height;
  ...
  Material.prototype.bindAlphaPrepass = function (a) {
    ...
    var b = this.gl, c = this.prepassShader.params, d = this.prepassShader.samplers;
  ...
  Material.prototype.bindWire = function (a) {
    if (!this.complete()) return !1;
    var b = this.gl, c = this.wireShader.params, d = a.view;
    b.enable(b.BLEND);
  ...
  var Matrix = {
    type: Float32Array, create: function (a, b, c, d, e, f, g, h, k, l, m, n, r, p, q, u) {
      ...
      var d = b[0], e = b[1], f = b[2], g = b[3], h = b[4], k = b[5], l = b[6], m = b[7], n = b[8], r = b[9], p = b[10],
        q = b[11], u = b[12], s = b[13], z = b[14];
      b = b[15];
      ...
      var c = b[0], d = b[1], e = b[2], f = b[3], g = b[4], h = b[5], k = b[6], l = b[7], m = b[8], n = b[9], r = b[10],
        p = b[11], q = b[12], u = b[13], s = b[14], z = b[15], t = c * h - d * g, v = c * k - e * g, w = c * l - f * g,
        x = d * k - e * h, A = d * l - f * h, B = e * l - f * k, C = m * u - n * q, D = m * s - r * q,
        E = m * z - p * q, F = n * s - r * u, G = n * z - p * u, H = r * z - p * s,
      ...
      var e = a.subarray(0, 3), f = a.subarray(4, 7), g = a.subarray(8, 11);
      Vect.sub(g, b, c);
      Vect.cross(e, d, g);
      Vect.normalize(g, g);
      Vect.normalize(e, e);
  ...
  MeshRenderable.prototype.drawWire = function () {
    var a = this.material.wireShader.attribs, b = this.gl;
    b.enableVertexAttribArray(a.vPosition);
  ...
  var Network = {
    fetchImage: function (a, b, c) {
      ...
      console.log("fetchBinaryIncremental work!");
      var e = new XMLHttpRequest;
      e.open("HEAD", a, !0);
      e.onload = function () {
        if (200 == e.status) {
          ...
          console.log(f);
          if (f && "none" != f) {
            var g = e.getResponseHeader("Content-Length") | 0, h = function (c, e) {
  ...
  function PostRender(a, b, c) {
    ...
    this.sampleCount = 1;
    this.sampleIndex = 0;
    c && (c = [], this.gl.hints.mobile ? (this.sampleCount = 3, this.sampleOffsets = [[-0.4375, -0.5625], [0.625, -0.25], [-0.1875, 0.5]]) : (c.push("#define HIGHQ"), this.sampleCount = 4, this.sampleOffsets = [[-0.5, -0.5], [0.5, -0.5], [-0.5, 0.5], [0.5, 0.5]]), this.aaResolve = a.shaderCache.fromURLs("postvert.glsl", "aaresolve.glsl", c));
    ...
    if (this.useBloom) {
      ...
      for (c = 0; 2 > c; ++c) b = {
        ...
        this.bloomTargets[c] = new Framebuffer(a, {width: b.width, height: b.height, color0: this.bloomTextures[c]});
    ...
    this.noiseTexture =
      new Texture(this.gl, {width: 128, height: 128});
    ...
    this.blackTexture = new Texture(this.gl, {width: 1, height: 1});
  ...
  PostRender.prototype.computeParams = function (a, b) {
    ...
    var e = 1 / this.noiseTexture.desc.width, f = 1 / this.noiseTexture.desc.height, g = 1 - c.grainSharpness;
  ...
  PostRender.prototype.present = function (a, b, c, d) {
    ...
    if (e.bind()) {
      d = this.gl;
      var f = e.samplers, g = e.params, h = this.computeParams(b, c);
      a.bind(f.tInput);
  ...
  PostRender.prototype.allocAABuffers = function (a, b) {
    if (void 0 === this.sampleTextures || this.sampleTextures[0].desc.width != a || this.sampleTextures[0].desc.height != b) {
      ...
      for (var c = 0; c < this.sampleCount; ++c) {
        var d = new Texture(this.gl, {width: a, height: b, nofilter: !0});
        d.loadArray();
        this.sampleTextures.push(d);
        this.sampleFramebuffers.push(new Framebuffer(this.gl, {width: a, height: b, color0: d, ignoreStatus: !0}))
      }
  ...
  PostRender.prototype.adjustProjectionForSupersampling = function (a) {
    if (1 < this.sampleCount) {
      var b = this.sampleOffsets[this.sampleIndex][0] / a.size[0],
        c = this.sampleOffsets[this.sampleIndex][1] / a.size[1], b = Matrix.translation(Matrix.empty(), b, c, 0);
  ...
  Shader.prototype.build = function (a, b) {
    ...
    for (var e = c.getProgramParameter(this.program, c.ACTIVE_UNIFORMS), f = 0, d = 0; d < e; ++d) {
      var g = c.getActiveUniform(this.program, d), h = g.name, k = h.indexOf("[");
      0 <= k && (h = h.substring(0,
        k));
  ...
  ShaderCache.prototype.fromURLs = function (a, b, c) {
    ...
    var f = new Shader(this.gl), g = null, h = null, k = function () {
  ...
  ShaderCache.prototype.resolveIncludes = function (a, b) {
    for (var c = [], d = !0, e = function (a, b, e, f, m) {
      d = !0;
      c.push({offset: m, path: b.slice(1, b.length - 1)});
      return ""
  ...
  function ShadowCollector(a, b) {
    ...
    if (0 < this.shadowCount) {
      var c = {width: 2048, height: 2048, clamp: !0, mipmap: !1, nofilter: !0};
      a.hints.mobile && (c.width = c.height = 1536);
      for (var d = {
        width: c.width, height: c.height, depthBuffer: Framebuffer.createDepthBuffer(a,
          c.width, c.height)
  ...
  ShadowCollector.prototype.collect = function (a, b) {
    var c = this.gl, d = a.lights, e = d.shadowCount, f = d.modelViewBuffer, g = d.projectionBuffer, h = d.matrix;
    if (!(0 >= e)) {
      for (var k = Matrix.empty(), l = !1, m = 0; m < e; ++m) if (d.shadowsNeedUpdate[m]) {
        ...
        n.bind();
        c.uniformMatrix4fv(n.params.uViewProjection,
          !1, k);
        for (var r = 0; r < a.meshRenderables.length; ++r) {
          var p = a.meshRenderables[r], q = p.material;
  ...
  function Sky(a, b, c) {
    ...
    if (void 0 !== d) {
      this.specularTexture = new Texture(a, {width: 256, height: 2048, clamp: !0});
      b = d.data;
    ...
    if (1 <= this.backgroundMode) if (this.backgroundShader = a.shaderCache.fromURLs("skyvert.glsl", 3 == this.backgroundMode ? "skySH.glsl" : "sky.glsl", ["#define SKYMODE " + this.backgroundMode]), this.vertexBuffer = a.createBuffer(), a.bindBuffer(a.ARRAY_BUFFER, this.vertexBuffer), c = 1 / 256, b = 0.5 / 256, d = 2.8 * b, e = 0.5 * b, c = new Float32Array([0, 1, 0, 0.49609375 + c, 0.49609375 + c, 1, 0, 0, 0.9921875 + c, 0.49609375 + c, 0, 0, 1, 0.49609375 + c, 0.9921875 + c, -1, 0, 0, 0 + c, 0.49609375 + c, 0, 0, -1, 0.49609375 + c, 0 + c, 0, -1, 0, 0.9921875 +
      ...
      c, 0, 1, 0, 0.49609375 + c, 0.49609375 + c - e, 0, 1, 0, 0.49609375 + c + e, 0.49609375 + c, 0, 1, 0, 0.49609375 + c, 0.49609375 + c + e]), a.bufferData(a.ARRAY_BUFFER, c, a.STATIC_DRAW), a.bindBuffer(a.ARRAY_BUFFER, null), this.indexBuffer = a.createBuffer(), a.bindBuffer(a.ELEMENT_ARRAY_BUFFER, this.indexBuffer), c = new Uint16Array([2, 1, 6, 3, 2, 7, 8, 4, 3, 4, 5, 1, 9, 14, 15, 17, 10, 16, 18, 19, 11, 20, 13, 12, 28, 12, 13, 13, 24, 28, 28, 24, 9, 9, 24, 14, 25, 9, 15, 25, 15, 21, 10, 25, 21, 10, 21, 16, 22, 26, 10, 22, 10, 17, 18, 11, 26, 22, 18, 26, 19, 23, 27, 19, 27, 11, 23, 20, 27, 27, 20, 12]), this.skyIndexCount =
        c.length, a.bufferData(a.ELEMENT_ARRAY_BUFFER, c, a.STATIC_DRAW), a.bindBuffer(a.ELEMENT_ARRAY_BUFFER, null), 3 == this.backgroundMode) for (this.backgroundCoefficients = new Float32Array(this.diffuseCoefficients), g = 0; g < this.backgroundCoefficients.length; ++g) this.backgroundCoefficients[g] *= this.backgroundBrightness; else {
      this.backgroundTexture = new Texture(a, {width: 256, height: 256, clamp: !0});
      c = !1;
      var k;
      a.ext.textureHalf && a.ext.textureHalfLinear && (this.backgroundTexture.loadArray(null, a.RGB, a.ext.textureHalf.HALF_FLOAT_OES),
        k = new Framebuffer(a, {color0: this.backgroundTexture}), c = k.valid);
      !c && a.ext.textureFloat && a.ext.textureFloatLinear && !a.hints.mobile && (this.backgroundTexture.loadArray(null, a.RGB, a.FLOAT), k = new Framebuffer(a, {color0: this.backgroundTexture}), c = k.valid);
      c || (this.backgroundTexture.loadArray(), k = new Framebuffer(a, {color0: this.backgroundTexture}));
      k.bind();
      k = new Shader(a);
  ...
  Sky.prototype.setClearColor = function () {
    if (marmoset.transparentBackground) this.gl.clearColor(0, 0, 0, 0); else if (1 > this.backgroundMode) {
  ...
  Sky.prototype.draw = function (a) {
    ...
    if (this.complete()) {
      var b = this.gl, c = this.backgroundShader, d = a.view, e = a.lights.invMatrix;
      c.bind();
  ...
  StripData.prototype.update = function (a) {
    ...
    for (var c = !1, d = 0; d < this.stripCount; ++d) {
      ...
      if (a) this.strips[d] = e; else {
        var f = e - this.strips[d], f = f * StripData.expDecay(0.05, b);
  ...
  StripData.prototype.activeFade = function () {
    var a = (this.strips[this.stripCount - 1] - -2) / (-0.9 + 0.3 * this.stripCount - -2), a = 1 < a ? 1 : a;
    return 0 > a ? 0 : a
  };
  ...
  TextureCache.prototype.fromFilesMergeAlpha = function (a, b, c) {
    ...
    var d = a.name + "|" + b.name, e = this.cache[d];
    if (void 0 !== e) return e;
    var f = this.gl;
    ...
    this.cache[d] = g;
    var h = 0, k = 0, l = this.mergeShader, m = this.mergeVerts, n = function () {
      if (h && k) {
        var a = h.width > k.width ? h.width : k.width, b = h.height > k.height ? h.height : k.height;
        g.desc.width = a;
        g.desc.height = b;
...
/* modify function TextureCache.parseFile to get an blob object instead of blob:URL*/
  TextureCache.parseFile1 = function (a, b) {
    a = new Blob([a.data], {type: a.type});
    b&&b(a);
  };
  ...
  UI.prototype.setThumbnail = function (a) {
    if (this.thumbnail) {
      var b = this.thumbnail.getContext("2d"), c = this.thumbnail.width, d = this.thumbnail.height, e = d / a.height;
      ...
      for (e = 0; 3 > e; ++e) {
        for (var h = f.data, k = a.data, l = 0, m = 0; m < d; ++m) for (var n = 0; n < c; ++n) {
          for (var r = 0, p = 0, q = 0, u = -2; 2 >= u; ++u) for (var s = m + u, s = 0 > s ? 0 : s >= d ? d - 1 : s, z = -2; 2 >= z; ++z) var t = n + z, t = 0 > t ? 0 : t >= c ? c - 1 : t, t = 4 * (s * c + t), r = r + h[t], p = p + h[t + 1], q =
            q + h[t + 2];
          k[l++] = r / 25;
          k[l++] = p / 25;
          k[l++] = q / 25;
  ...
  UI.prototype.showActiveView = function () {
    ...
    if (!marmoset.noUserInterface) {
      ...
      var b = FullScreen.support(), b = !0, a = 1;
      ...
      var g = d.title && 0 < d.title.length, e = d.subtitle && 0 < d.subtitle.length,
        f = d.author && 0 < d.author.length, h = d.link && 0 < d.link.length;
      if (g || e || f) {
        g || (d.title = "Art");
        var k = !g && !e, l = document.createElement("div");
  ...
  UI.prototype.refreshUI = function () {
    if (this.sigCluster) {
      var a = !1, b = this.stripData.selectedStrip == this.stripData.STRIP_MENU;
  ...
  UI.prototype.animate = function () {
    ...
    if (this.frameTimer && (this.frameCount++, 60 <= this.frameCount)) {
      ...
      if (void 0 !== this.frameTime) {
        var b = (a - this.frameTime) / this.frameCount, b = Math.floor(100 *
          b) / 100;
  ...
  UI.prototype.animateStrips = function () {
    if (this.stripText) for (var a = Math.atan(this.viewer.canvas.height / this.viewer.canvas.width / this.stripData.stripSlant), b = 0; b < this.stripData.labels.length; ++b) {
      var c = this.stripData.strips[b], c = c - this.stripData.stripSlant, c = 0.5 + 0.5 * c;
  ...
  var Vect = {
    type: Float32Array, create: function (a, b, c, d) {
      ...
      var c = b[0], d = b[1], e = b[2], f = Math.sqrt(c * c + d * d + e * e);
  ...
  View.prototype.updateView = function () {
    if (void 0 !== this.limits) {
      if (this.limits.angles) {
        var a = this.limits.angles.x, b = this.limits.angles.y;
        if (void 0 !== a) {
          var c = this.rotation[0] - a.offset, a = Math.min(Math.max(c, a.min), a.max);
          this.rotation[0] += a - c
        }
  ...
  WebViewer.prototype.allocBacking = function () {
    var a = this.gl, b = !1, c = {width: this.canvas.width, height: this.canvas.height};
    ...
    !b && a.ext.textureFloat && a.ext.textureFloatLinear && !a.hints.mobile && (this.mainColor.loadArray(null, a.RGBA, a.FLOAT), this.mainBuffer = new Framebuffer(a,
      {color0: this.mainColor, createDepth: !0}), b = this.mainBuffer.valid);
  ...
  WebViewer.prototype.loadScene = function (a) {
    ...
    if (this.initGL() && this.sceneURL) {
      ...
      a = function (a) {
        ...
        if (this.scene.load(new Archive(a))) if (2070 >= this.scene.metaData.tbVersion) this.ui.showFailure("This .mview file is from an out-of-date beta version of Toolbag. Please re-export it with the new version. Thanks!"); else {
          if (this.bindInput(),
  ...
  WebViewer.prototype.unload = function () {
    ...
    delete this.gl;
    var a = this.domRoot.clientWidth, b = this.domRoot.clientHeight;
    this.initCanvas(a, b);
    this.ui = new UI(this);
    this.ui.setSize(a, b);
    this.ui.showPreview();
  ...
  WebViewer.prototype.bindInput = function () {
    ...
    this.input.onPan.push(function (b, c) {
      var d = this.scene.view, e = d.fov / 45 * 0.8 *
        (d.radius / this.domRoot.clientHeight), f = -b * e, e = c * e;
